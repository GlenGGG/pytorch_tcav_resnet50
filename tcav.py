...
from cav import CAV
from cav import get_or_train_cav, get_cav
import run_params
import utils
import numpy as np
import time
...
class TCAV(object):
    ...
    @staticmethod
    def get_direction_dir(mymodel, act, cav, concept, class_id):
        """Get the sign of directional derivative.
        Args:
            mymodel: a model class instance
            act: activations of one bottleneck to get gradient with respect to.
    ...
    def compute_sensitivity(
        ...
        if run_parallel:
            ...
            directions = pool.map(
                lambda i: TCAV.get_direction_dir(
                    mymodel,
                    np.expand_dims(class_acts[i], 0),
                    cav,
                    concept,
                    class_id,
                ),
            ...
            for i in range(len(class_acts)):
                ...
                count += TCAV.get_direction_dir(
                    mymodel, act, cav, concept, class_id
                )
    ...
    def _run_single_set(self, param, overwrite=False, run_parallel=False):
        ...
        result = {
            ...
            "i_up": i_up,
            "val_directional_dirs_abs_mean": np.mean(
                np.abs(val_directional_dirs)
            ),
    ...
    def _process_what_to_run_expand(
        self, num_random_exp=100, random_concepts=None
    ):
        ...
        (
            ...
            - (
                1
                if random_concepts
                and self.random_counterpart in random_concepts
                else 0
            )
        ...
        if self.random_counterpart is None:
            ...
            (
                ...
                utils.process_what_to_run_randoms(
                    target_concept_pairs, self.random_counterpart
                ),
                self.random_counterpart,
                num_random_exp=num_random_exp
                - (
    ...
    @staticmethod
    def get_params_from_concepts_and_target(
        target, concept, bottleneck, alpha, random_exp_num
    ):
        params = []

        def get_random_concept(i):
            return "random500_{}".format(i)

        pairs_to_test = [
            (target, [concept, get_random_concept(i)])
            for i in range(random_exp_num)
        ]
        # print(
        #     "target: {}, concept: {}, pairs_to_test: {}".format(
        #         target, concept, pairs_to_test
        #     )
        # )

        for target_in_test, concepts_in_test in pairs_to_test:
            tf.compat.v1.logging.info(
                "%s %s %s %s",
                bottleneck,
                concepts_in_test,
