...
class CAV(object):
    ...
    def _create_cav_training_set(concepts, bottleneck, acts):
        ...
        for i, concept in enumerate(concepts):
            x.extend(
                acts[concept][bottleneck][:min_data_points].reshape(
                    min_data_points, -1
                )
            )
    ...
    def train(self, acts):
        ...
        tf.compat.v1.logging.info(
            "training with alpha={}".format(self.hparams["alpha"])
        )
        ...
        if self.hparams["model_type"] == "linear":
            ...
            raise ValueError(
                "Invalid hparams.model_type: {}".format(
...
def get_or_train_cav(
    concepts, bottleneck, acts, cav_dir=None, cav_hparams=None, overwrite=False
):
    ...
    if cav_dir is not None:
        ...
        if not overwrite and tf.io.gfile.exists(cav_path):
            ...
            tf.compat.v1.logging.info(
                "CAV accuracies: {}".format(cav_instance.accuracies)
            )
            return cav_instance

    tf.compat.v1.logging.info(
        "Training CAV {} - {} alpha {}".format(
...
def get_cav(concepts, bottleneck, cav_dir=None, cav_hparams=None):
    if cav_hparams is None:
        cav_hparams = CAV.default_hparams()
    ...
    if cav_dir is not None:
        ...
        if tf.io.gfile.exists(cav_path):
            tf.compat.v1.logging.info("CAV already exists: {}".format(cav_path))
            cav_instance = CAV.load_cav(cav_path)
            tf.compat.v1.logging.info(
                "CAV accuracies: {}".format(cav_instance.accuracies)
